-- Preserve distinct business locations from the master sheet even when they share a phone number.
-- The client supplies a normalized business + city + address key. Source row IDs still make reimports idempotent.

create or replace function public.set_cold_lead_integrity_fields()
returns trigger language plpgsql set search_path = public as $$
begin
  if not (new.source_system = 'google_sheets' and new.dedupe_key like 'business:%') and
     (new.dedupe_key is null or
...
begin
  ...
  loop
    ...
    candidate.dedupe_key := coalesce(nullif(candidate.dedupe_key, ''),
      public.cold_lead_identity_key(candidate.email, candidate.whatsapp, candidate.phone, candidate.company, candidate.city));
    ...
    where lead.archived_at is null
      and (
        ...
        or lead.dedupe_key = candidate.dedupe_key
      )
    order by (
      candidate.source_external_id is not null
      and lead.source_system = coalesce(candidate.source_system, 'google_sheets')
...
comment on function public.import_cold_leads(jsonb, text) is
  'Idempotently imports the Wuaya master sheet while preserving distinct business locations.';
