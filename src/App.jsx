...
function FileCard({file,onDelete,showDel=true}){
  ...
  const handleDL=async()=>{if(file.url){window.open(file.url,"_blank");return}if(file.storage_path){const{data}=supabase.storage.from("wauya-files").getPublicUrl(file.storage_path);if(data?.publicUrl)window.open(data.publicUrl,"_blank")}};
...
export default function App(){
  ...
  const loadAll=useCallback(async()=>{
    ...
    if(p.data)setProspects(p.data);if(cl.data)setClients(cl.data);if(em.data)setEmployees(em.data);if(cu.data)setCU(cu.data);if(td.data)setTodos(td.data);if(fl.data)setFiles(fl.data);
  },[]);
  ...
  const uploadFile=async(file,ownerType,ownerId,fileType)=>{
    setSaving(true);const ext=file.name.split(".").pop();const path=`${ownerType}/${ownerId}/${Date.now()}.${ext}`;
  ...
  const deleteFile=async(fileId)=>{const f=files.find(x=>x.id===fileId);if(f?.storage_path)await supabase.storage.from("wauya-files").remove([f.storage_path]);await dbDel("files",fileId)};
  ...
  if(user?.role==="employee"){
    const myTodos=todos.filter(td=>td.assigned_to===user.id);const grouped={};myTodos.forEach(td=>{const cl=clients.find(x=>x.id===td.client_id);const k=cl?.id||"x";if(!grouped[k])grouped[k]={client:cl,todos:[]};grouped[k].todos.push(td)});
  ...
  return<div style={{fontFamily:F,background:C.bg,minHeight:"100vh",display:"flex"}}><style>{CSS}</style>
    ...
    <div style={{flex:1,overflow:"auto"}}><div style={{padding:"24px 32px",animation:"fadeIn .25s ease"}}>
      ...
      {view==="settings"&&<SettingsView user={user} onSave={async(updates)=>{
        setSaving(true);await supabase.from("admin_users").update(updates).eq("id",user.id);
        const newUser={...user,...updates};delete newUser.password;setUser(newUser);localStorage.setItem("wauya_user",JSON.stringify(newUser));
